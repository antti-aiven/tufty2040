...
# Define text to show
your_name = "allan stojanovic"
slogan = "Anti-Abuse Operations!"
...
while True:
    ...
    if (ball_y in range(y, y + 40)) and (
        ball_x in range(x, x + 120)
    ):  # Change direction if you hit boundary
        burst = 0
        display.set_pen(BLACK)
        display.clear()
        BOUNCE_COLOR = display.create_pen(
    ...
    display.text(your_name, 20, 70, 200, 5)
    display.set_pen(BLUE)
    display.text(your_name, 22, 72, 200, 5)
    display.set_pen(WHITE)
    display.text(your_name, 24, 74, 200, 5)
