...
def get_applications():
    ...
    for file in listdir():
        if file.endswith(".py") and file != "main.py":
            # convert the filename from "something_or_other.py" to 
"Something Or Other"
            # via weird incantations and a sprinkling of voodoo
            title = " ".join([v[:1].upper() + v[1:] for v in 
file[:-3].split("_")])
    ...
    # sort the application list alphabetically by title and return the 
list
...
while True:
    ...
    if button_up.read():
        ...
        target_scroll_position = target_scroll_position if 
target_scroll_position >= 0 else len(applications) - 1

    if button_down.read():
        target_scroll_position += 1
        target_scroll_position = target_scroll_position if 
target_scroll_position < len(applications) else 0
    ...
    for y in range(0, 240 / grid_size):
        for x in range(0, 320 / grid_size):
            ...
            display.rectangle(x * grid_size, y * grid_size, grid_size, 
grid_size)
            
    # work out which item is selected (closest to the current scroll 
position)
    ...
    for list_index, application in enumerate(applications):
        ...
        title_width = display.measure_text(application["title"], 
text_size)
        ...
        if selected_item == list_index:
          text(application["title"], text_x + 1, text_y + 1, shadow_pen, 
text_size)
          
        text_pen = selected_pen if selected_item == list_index else 
unselected_pen
